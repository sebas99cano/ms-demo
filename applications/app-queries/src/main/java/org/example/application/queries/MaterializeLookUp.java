...
import org.example.application.queries.adapter.repo.WalletModelView;
...
public class MaterializeLookUp {
    ...
    public MaterializeLookUp(ApplicationContext context, AccountRepository repository) {
        business.put("org.example.AccountCreated", Flux.just(input -> {
            ...
            document.setWalletModelViewById(event.getWalletId().value());
        ...
        business.put("org.example.CoinInWalletAdded", Flux.just(input -> {
            var event = (CoinInWalletAdded) input;
            return repository.findById(event.aggregateRootId()).flatMap(doc -> {
                var wallet = doc.getWalletModelView();
                wallet.setCoin(wallet.getCoin() + event.getCoin().value());
                doc.setWalletModelView(wallet);
                return repository.save(doc).then();
            });
        }));
    }
